...
- Doctor role permissions are edited from **Doctors → Doctor permissions** (per branch) via `GET/PUT /api/v1/roles/system/doctor`; applies to all doctors in that branch. You can also use `PUT /api/v1/roles/:id/permissions` with the doctor role id.
